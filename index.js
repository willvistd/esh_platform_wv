...
async function initDB() {
  ...
  if (parseInt(eduTypesExisting.rows[0].count) === 0) {
    const eduTypeSeed = [
      ...
      ['작업내용변경_근로자',        '작업내용 변경 시 교육 (근로자)',        '2시간',
       '산업안전보건법 시행규칙 [별표 5] 제1호 다목 (채용 시 교육 및 작업내용 변경 시 교육)\n○ 산업안전 및 산업재해 예방에 관한 사항(화재ㆍ폭발 사고 발생 시 대피에 관한 사항을 포함한다)\n○ 산업보건 및 건강장해 예방에 관한 사항\n○ 위험성 평가에 관한 사항\n○ 산업안전보건법령 및 산업재해보상보험 제도에 관한 사항\n○ 직무스트레스 예방 및 관리에 관한 사항\n○ 기계ㆍ기구의 위험성과 작업의 순서 및 동선에 관한 사항\n○ 작업 개시 전 점검에 관한 사항\n○ 정리정돈 및 청소에 관한 사항\n○ 사고 발생 시 긴급조치에 관한 사항\n○ 물질안전보건자료에 관한 사항', 30],
      ['고객응대_건강보호',         '고객응대근로자 건강보호 교육',           '2시간',
       '산업안전보건법 제41조 (고객응대근로자 건강장해 예방)\n○ 고객응대업무 매뉴얼의 내용 및 활용 방법\n○ 고객응대업무로 인한 건강장해 예방 방법\n[폭력 예방 및 대처]\n○ 공격 등 폭력행동 및 상황에 대한 빠른 인지 방법\n○ 폭력상황·공격적 행동·분노를 누그러뜨리는 방법\n○ 비상경보 및 연락체계 수립·신속한 연락방법\n○ 공격적인 사람과 폭력상황에 침착하게 대처하는 방법\n○ 폭력사건 보고·기록 유지 절차 및 방법\n○ 의학적 치료·상담, 폭력사건·손상 후 보상 및 법적 조치 안내', 35],
  ...
  // ── (1회) 사용 안 하는 교육종류 '특별교육 개별 (제32호 압력용기)' 제거 ──
  // 이미 DB에 시드된 항목을 정리. app_settings 마커로 1회만 실행 →
  // 이후 관리화면에서 사용자가 다시 추가하면 그대로 유지됨(재삭제 안 함).
  try {
    const doneP = await pool.query("SELECT value FROM app_settings WHERE key='edu_remove_pressure_vessel_v1'");
    if (doneP.rowCount === 0) {
      const del = await pool.query("DELETE FROM education_types WHERE id='특별교육_압력용기' RETURNING id");
      await pool.query("INSERT INTO app_settings (key, value) VALUES ('edu_remove_pressure_vessel_v1', NOW()::TEXT) ON CONFLICT (key) DO NOTHING");
      if (del.rowCount) console.log("[DB] 교육종류 '특별교육_압력용기'(제32호 압력용기) 제거 완료");
    }
  } catch (e) { console.error("교육종류 압력용기 제거 실패:", e); }

  // ── (1회) 특별교육 공통내용(근로자) 교육내용 문구 보정: '(특별교육 공통내용)' → '(특별교육 공통내용 - 근로자)' ──
  try {
    const doneW = await pool.query("SELECT value FROM app_settings WHERE key='edu_common_worker_label_v1'");
    if (doneW.rowCount === 0) {
      await pool.query(
        `UPDATE education_types
           SET content = REPLACE(content, '제3호 가목 (특별교육 공통내용)', '제3호 가목 (특별교육 공통내용 - 근로자)')
         WHERE id='특별교육_공통_근로자'`
      );
      await pool.query("INSERT INTO app_settings (key, value) VALUES ('edu_common_worker_label_v1', NOW()::TEXT) ON CONFLICT (key) DO NOTHING");
  ...
  // ── (1회) 고객응대근로자 건강보호 교육(산안법 제41조) 종류 추가 ──
  try {
    const doneC = await pool.query("SELECT value FROM app_settings WHERE key='edu_add_customer_worker_v1'");
    if (doneC.rowCount === 0) {
      await pool.query(
        `INSERT INTO education_types (id, label, hours, content, "sortOrder") VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO NOTHING`,
        ['고객응대_건강보호', '고객응대근로자 건강보호 교육', '2시간',
         '산업안전보건법 제41조 (고객응대근로자 건강장해 예방)\n○ 고객응대업무 매뉴얼의 내용 및 활용 방법\n○ 고객응대업무로 인한 건강장해 예방 방법\n[폭력 예방 및 대처]\n○ 공격 등 폭력행동 및 상황에 대한 빠른 인지 방법\n○ 폭력상황·공격적 행동·분노를 누그러뜨리는 방법\n○ 비상경보 및 연락체계 수립·신속한 연락방법\n○ 공격적인 사람과 폭력상황에 침착하게 대처하는 방법\n○ 폭력사건 보고·기록 유지 절차 및 방법\n○ 의학적 치료·상담, 폭력사건·손상 후 보상 및 법적 조치 안내', 35]
      );
      await pool.query("INSERT INTO app_settings (key, value) VALUES ('edu_add_customer_worker_v1', NOW()::TEXT) ON CONFLICT (key) DO NOTHING");
      console.log("[DB] 교육종류 '고객응대근로자 건강보호 교육' 추가 완료");
    }
  } catch (e) { console.error("고객응대근로자 교육종류 추가 실패:", e); }

  // ── (1회) 고객응대근로자 건강보호 교육내용 갱신 v2 (매뉴얼 문구 정리, 추가 교육내용 섹션 제거) ──
  try {
    const doneC2 = await pool.query("SELECT value FROM app_settings WHERE key='edu_customer_worker_content_v2'");
    if (doneC2.rowCount === 0) {
      await pool.query(
        `UPDATE education_types SET content=$1, "updatedAt"=NOW()::TEXT WHERE id='고객응대_건강보호'`,
        ['산업안전보건법 제41조 (고객응대근로자 건강장해 예방)\n○ 고객응대업무 매뉴얼의 내용 및 활용 방법\n○ 고객응대업무로 인한 건강장해 예방 방법\n[폭력 예방 및 대처]\n○ 공격 등 폭력행동 및 상황에 대한 빠른 인지 방법\n○ 폭력상황·공격적 행동·분노를 누그러뜨리는 방법\n○ 비상경보 및 연락체계 수립·신속한 연락방법\n○ 공격적인 사람과 폭력상황에 침착하게 대처하는 방법\n○ 폭력사건 보고·기록 유지 절차 및 방법\n○ 의학적 치료·상담, 폭력사건·손상 후 보상 및 법적 조치 안내']
      );
      await pool.query("INSERT INTO app_settings (key, value) VALUES ('edu_customer_worker_content_v2', NOW()::TEXT) ON CONFLICT (key) DO NOTHING");
      console.log("[DB] 고객응대근로자 교육내용 갱신(v2) 완료");
    }
  } catch (e) { console.error("고객응대근로자 교육내용 갱신 실패:", e); }
...

{
  ...
  "revisionDate": "MSDS 최종 개정일(개정일자/작성일). 보통 문서 마지막 부분(16절 '그 밖의 참고사항' 등)에 표기됨. YYYY-MM-DD 형식으로 반환. 개정일과 작성일이 모두 있으면 가장 최근(개정)일. 없으면 빈 문자열",
  ...
  "components": [MSDS 3절 '구성성분의 명칭 및 함유량' 표의 각 성분을 객체로. 형식: {"name":"국문 성분명","cas":"CAS번호(예:108-88-3, 없으면 빈문자열)","content":"함유량 원문 그대로(예:30~40%, <1%, 영업비밀)"}. 표에 있는 모든 성분을 빠짐없이. 영업비밀·기타 표기도 name에 그대로 넣고 content는 원문대로]
}
